...
function EXECCOMMAND(cmd) {
	...
	if (cmd == 20220301) {
		SRVLINES.FILTER = '({SRVLINES.QTY1} > 0 OR {SRVLINES.QTY} > 0 OR {SRVLINES.QTY2} > 0)';
		SRVLINES.FILTERED = !SRVLINES.FILTERED;
	}
	...
	if (cmd == 20190515) {
		...
		sliceTime(CCCANGAJATISUBATREPRENOR);
		//return;
		var tot = hProgramTot();
		calculOreActivitati();
		X.EXEC('button:Save');
			...
			if (SRVLINES.QTY > 0 && !SRVLINES.BOOL02) {
				...
				try {
					...
					if (r) {
						...
						tblHeader.Edit;
						tblHeader.TRNDATE = SALDOC.DATE02;
						tblHeader.DATE01 = SALDOC.DATE02;
						if (SRVLINES.QTY)
						...
						while (!CCCANGAJATISUBATREPRENOR.EOF) {
							tblLinii.Append;
							tblLinii.CDAT01 = CCCANGAJATISUBATREPRENOR.DELA;
							tblLinii.CDAT02 = CCCANGAJATISUBATREPRENOR.PANALA;
							tblLinii.CINT01 = CCCANGAJATISUBATREPRENOR.PRSN;
							var M11 = dateDiffH(X.FORMATDATE('YYYY-MM-DD HH:MM', CCCANGAJATISUBATREPRENOR.DELA), X.FORMATDATE('YYYY-MM-DD HH:MM', CCCANGAJATISUBATREPRENOR.PANALA), CCCANGAJATISUBATREPRENOR.PONTAJZI);
							var L6 = SRVLINES.QTY;
							//l6*m11/tot
							tblLinii.CNUM01 = L6 * M11 / totCuExcl;
							tblLinii.CINT05 = SALDOC.FINDOC; //FINDOCS like
							if (CCCANGAJATISUBATREPRENOR.PONTAJZI) {
								tblLinii.CINT03 = 100;
								tblLinii.CINT09 = 1;
							} else {
								tblLinii.CINT03 = 101;
...
function hProgramTot() {
	...
	CCCANGAJATISUBATREPRENOR.FIRST;
	while (!CCCANGAJATISUBATREPRENOR.EOF) {
		var dif = dateDiffH(X.FORMATDATE('YYYY-MM-DD HH:MM', CCCANGAJATISUBATREPRENOR.DELA), X.FORMATDATE('YYYY-MM-DD HH:MM', CCCANGAJATISUBATREPRENOR.PANALA), CCCANGAJATISUBATREPRENOR.PONTAJZI);
...
function calculOreActivitati() {
	...
	*/
	...
	hNormatTot = X.FILTERSUM('SRVLINES.QTY2', 'CCCUM <> 6')
...
/*orar = CCCANGAJATISUBATREPRENOR:
06:00	22:00	PREZENTA(ZI)	1a-Executie lucrari (zi)
22:00	06:00	PREZENTA ( NOAPTE)	1c-Executie lucrari (noapte)
Pauza de masa se aplica la fiecare interval 6-22-6
*/
function sliceTime(orar) {
	//debugger;
	var orarOrig = [],
		pontaje = [];
	orar.first;
	while (!orar.eof) {
		var linie = {};
		linie.PRSN = orar.PRSN;
		linie.DELA = new Date(X.FORMATDATE('YYYY-MM-DD', orar.DELA) + 'T' + X.FORMATDATE('HH:mm:ss', orar.DELA));
		linie.PANALA = new Date(X.FORMATDATE('YYYY-MM-DD', orar.PANALA) + 'T' + X.FORMATDATE('HH:mm:ss', orar.PANALA));
		orarOrig.push(linie);
		pontaje.push.apply(pontaje, getIntervalsFromLine(orarOrig[orarOrig.length - 1].PRSN, orarOrig[orarOrig.length - 1].DELA, orarOrig[orarOrig.length - 1].PANALA));
		orar.next;
	}
	...
	if (pontaje.length) {
		...
		var strM = '';
		//debugger;
		for (var i = 0; i < pontaje.length; i++) {
			//strM += pontaje[i].prsn + '/' + dateToSQLStr(pontaje[i].dela, '-', ':') + '/' + dateToSQLStr(pontaje[i].panala, '-', ':') + '\n';
			orar.Append;
			orar.PRSN = pontaje[i].prsn;
			orar.DELA = dateToSQLStr(pontaje[i].dela, '-', ':');
			orar.PANALA = dateToSQLStr(pontaje[i].panala, '-', ':');
			orar.PONTAJZI = pontaje[i].zi;
			orar.Post;
		}
	}
...
//--> d 00:00 - d 06:00 - d 22:00 - d+1 00:00-->
function getIntervalsFromLine(PRSN, d1, d2) {
	var a = 'T00:00',
		b = 'T06:00',
		c = 'T22:00',
		i = 0,
	...
	intervals = loadIntervals(d1, i);
	loop1:
		for (var j = 0; j < intervals.length; j++) {
			if (checkDateBtwDT(d1, intervals[j], intervals[j + 1])) {
				pontaje.push({
					prsn: PRSN,
					dela: d1,
					panala: intervals[j + 1],
					zi: j == 1 ? 1 : 0
				});
				loop2:
					for (var k = j; k < intervals.length - 1; k++) {
						//restul zilei, pornind de la startul tocmai gasit
						if (checkDateBtwDT(d2, intervals[k], intervals[k + 1])) {
							//...
							foundIt = true;
							if (k == j) {
								pontaje.pop();
	...
	while (!foundIt) {
		//avoid stackoverflow on some kind of error; funtioneaza si ca limita de procesare (i=2 inseamna interval de doua zile)
		if (i > 100)
			return;

		//zilele urmatoare
		i++; //inca o zi
		//incarca ziua nou obtinuta
		intervals = loadIntervals(d1, i);
		//cauta in ea unde este d2
		for (k = 0; k < intervals.length - 1; k++) {
			//restul zilei, pornind de la startul tocmai gasit
			if (checkDateBtwDT(d2, intervals[k], intervals[k + 1])) {
				//...
				foundIt = true;
	...
	function loadIntervals(dayRef, i) {
		var intervals = [],
			currDay = new Date(dayRef),
			nextDay = new Date(dayRef);
		currDay.setDate(currDay.getDate() + i);
		nextDay.setDate(nextDay.getDate() + i + 1);
		intervals.push(new Date(currDay.getFullYear() + '-' + ('0' + (currDay.getMonth() + 1)).slice(-2) + '-' + ('0' + (currDay.getDate())).slice(-2) + a));
		intervals.push(new Date(currDay.getFullYear() + '-' + ('0' + (currDay.getMonth() + 1)).slice(-2) + '-' + ('0' + (currDay.getDate())).slice(-2) + b));
		intervals.push(new Date(currDay.getFullYear() + '-' + ('0' + (currDay.getMonth() + 1)).slice(-2) + '-' + ('0' + (currDay.getDate())).slice(-2) + c));
		intervals.push(new Date(nextDay.getFullYear() + '-' + ('0' + (nextDay.getMonth() + 1)).slice(-2) + '-' + ('0' + (nextDay.getDate())).slice(-2) + a));
		return intervals;
	}
}